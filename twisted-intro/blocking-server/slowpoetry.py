...
def send_poetry(sock, poetry_file, num_bytes, delay):
    ...
    while True:
        in_bytes = inputf.read(num_bytes)
        print(num_bytes, type(in_bytes), in_bytes)

        if not in_bytes:  # no more poetry :(
            sock.close()
            inputf.close()
            return

        print('Sending %d bytes' % len(in_bytes))

        try:
            sock.sendall(in_bytes.encode("utf-8"))  # this is a blocking call
        except socket.error:
            sock.close()
            inputf.close()
            return
